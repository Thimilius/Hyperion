...
namespace Hyperion {
    ...
    public:
        TextureAtlasPacker(uint32 width, uint32 height, uint32 padding, uint32 row_size) {
            ...
            m_pixel_buffer = m_pixel_data.data();
        }

        bool AddElement(K key, const V &value, uint32 element_width, uint32 element_height, uint8 *element_buffer) {
            ...
            // This means the y-coordinate (v) of the uvs starts at 1 and goes down instead of starting at 0 and going up.
            {
                ...
                element.uv_top_left = Vec2(left, top);
                element.uv_top_right = Vec2(right, top);
                element.uv_bottom_right = Vec2(right, bottom);
                element.uv_bottom_left = Vec2(left, bottom);
            }
        ...
        TextureAtlas<K, V> *CreateAtlas() {
            Texture2D *texture = Texture2D::Create(m_width, m_height, Rendering::TextureFormat::R8, Rendering::TextureParameters(), m_pixel_data);
            TextureAtlas<K, V> *texture_atlas = TextureAtlas<K, V>::Create(texture, m_elements);
    ...
    private:
        ...
        Vector<uint8> m_pixel_data;
        uint8 *m_pixel_buffer;
